...
未完成项仅为人工现场验证：目标 Dark Souls III 版本的 signature 命中、至少一次实际 Hook 安装、loose param、Wwise、BootBoost、独立存档、离线属性、Logo、白闪、mimalloc、regulation 保护，以及无 Arxan 中和时各 Hook 被保护、失效或恢复的实际行为。完成这些检查前不将阶段 9 标记为完成，也不宣称稳定支持。
...
`[darksouls3]`；Dark Souls III adapter 会安装实验性的共享 Host 与游戏适配 Hook，
但不包含 Arxan 中和，实际能力以状态日志和目标版本现场验证为准。全局选项包括 `debug`、`log_level`、`cpu_affinity`、
`reset_achievements_on_new_game` 和 `enable_ime`。游戏专用选项、外部 DLL 顺序和启动器参数已同步到仓库根目录的中英文 README。
...
README 为准。阶段 9 当前 Debug 构建已通过 31 项 CTest。VFS fake UID 采用稳定的
