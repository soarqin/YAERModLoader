...
Logo、离线属性、mimalloc allocator、regulation 保护和 native 功能仍需在内部 adapter capability 中继续拆分。
...
当前进展：已移除项目自定义扩展 ABI、对应 fixture 和初始化分支；内置扩展改为通过共享基础代码直接调用扫描和 Hook 能力，ModEngine 加载顺序及非 ER 跳过 ER 专用扩展的行为保留。Logo、离线属性、allocator、regulation 和 native capability 的跨游戏拆分，以及真实 Elden Ring 扩展和 ModEngine fixture 回归仍待完成。
