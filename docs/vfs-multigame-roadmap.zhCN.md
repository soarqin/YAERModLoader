...
本文记录 YAERModLoader 从「Elden Ring 专用加载器」演进为支持 Elden Ring、Sekiro: Shadows Die Twice 与 Dark Souls III 的 FromSoftware 模组加载器的设计、完成状态与后续计划。
...
- Elden Ring 与 Sekiro 具有独立 adapter；Dark Souls III 仍会在安装游戏专用 Hook 前停止。
- 配置解析会按当前进程选择 `[elden_ring]` 或 `[sekiro]`，但配置结构体仍复用同一组游戏字段。
...
当前配置模板位于 `src/YAERModLoader.ini`，发布包会将其复制为
`YAERModLoader.ini`。加载器按当前进程选择 `[elden_ring]`、`[sekiro]` 或实验性的
`[darksouls3]`；Dark Souls III section 会被解析，但 adapter 当前不会安装完整的
游戏专用 Hook。全局选项包括 `debug`、`log_level`、`cpu_affinity`、
`reset_achievements_on_new_game` 和 `enable_ime`。游戏专用选项、外部 DLL 顺序、
扩展配置文件和启动器参数已同步到仓库根目录的中英文 README。

`world_map_cursor_speed`、`auto_upgrade_weapons` 和 `itemlot_rate` 的选项属于
扩展 DLL 自己的配置文件，不属于 `YAERModLoader.ini`。ModEngine2 TOML fallback
仍按 Game Registry 选择 `config_eldenring.toml`、`config_sekiro.toml` 或
`config_darksouls3.toml`。

扩展配置的当前选项为：`er_param.ini` 的 `world_map_cursor_speed`，
`almighty_kale.ini` 的 `auto_upgrade_weapons`，以及 `itemlot_rate.ini` 的
`include_weapons`、`include_arrows`、`include_armors`、`include_goods` 和
`[alter_count]` 条目。

### 10.2 当前实现状态修正

前文阶段记录中的历史批次说明保留作为开发轨迹；当前代码和测试结果以本节及
README 为准。当前 Debug 构建已通过 33 项 CTest。VFS fake UID 采用稳定的
